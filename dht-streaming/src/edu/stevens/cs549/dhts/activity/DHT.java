...
//		routing.joinCheck();
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				 ...
				 */
				return client.findClosestPrecedingFinger(info, id);
			} else {
				/*
	...
	@Override
	public void sendTransferListener(){
		state.broadcastTransfer();
	}
...
//			state.dropBindings(info.id);
	 ...
	 */
	// Webmethod
	public EventOutput listenOnForBindings(int id, String key) {
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 */	
		NodeInfo node = getSucc(findPredecessor(NodeKey(key)));
		System.out.println(" DHT listenOn been called for " + key +"++");
