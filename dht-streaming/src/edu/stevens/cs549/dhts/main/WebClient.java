...
public class WebClient {
	...
	private Response delListenRequest(URI deletePath) {
		// TODO Auto-generated method stub
		try {
			Response cr = client.target(deletePath).request(MediaType.APPLICATION_XML_TYPE).delete();
			return cr;
		} catch (Exception e) {
			error("Exception during DELETE request: " + e);
			return null;
	...
	public EventSource listenForBindings(NodeInfo node, int id, String key) throws DHTBase.Failed {
		...
		URI listenOnPath = ub.queryParam("id", id).queryParam("key", key).build();
		System.out.println("WebClient.listenForBindings been called");
		info("client listenForBindings(" + listenOnPath + ")");
	...
	public void listenOff(NodeInfo node, int id, String key) throws DHTBase.Failed {
		UriBuilder ub = UriBuilder.fromUri(node.addr).path("listen");
		URI listenOffPath = ub.queryParam("id", id).queryParam("key", key).build();
		info("client listenOff(" + listenOffPath + ")");
		Response response = delListenRequest(listenOffPath);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("DELETE /Listen?id=ID");
		}
	}
	...
	public NodeInfo findClosestPrecedingFinger(NodeInfo info, int id) throws DHTBase.Failed {
		// TODO Auto-generated method stub
		UriBuilder ub = UriBuilder.fromUri(info.addr).path("findCPF");
		...
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /findCPF?id=ID");
		} else {
	...
	public void add(NodeInfo n, String k, String v) throws DHTBase.Failed {
		...
		TableRep t = new TableRep(null, null, 1);
		t.entry[0] = new TableRow(k, new String[] { v });
		info("client add(" + addPath + ")");
		Response response = putRequest(addPath, Entity.xml(t));
		if (response == null || response.getStatus() >= 300) {
